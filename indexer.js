...
async function bulkop(data, callback) {
...
function IndexMongodbData (esIndexName,esIndexType,collectionName)
{
    ...
    //   } else {
      ...
      //db.collection(collectionName, function (err, collection) {
        ...
        mongoose.connection.client.db("number-collection-dev").collection(collectionName).find({}).skip(offset).limit(limitData).sort( { _id: -1 } ).toArray(function(err, result) {
        if (result.length>0)
        {
